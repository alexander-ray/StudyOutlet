...
class Test {
    ...
    func getQuestions(completionHandler: @escaping ([question]?) -> Void) {
        // Endpoint url
        let url = "0.0.0.0:8080/question/questions"
        ...
        Alamofire.request(url).responseJSON { response in
            ...
            if((response.result.value) != nil) { // If there are available units
                ...
                for i in (0..<swifty.count) {
                    ...
                    if let q = Helper.base64ToImage(input: swifty[i]["question"].stringValue), let s = Helper.base64ToImage(input: swifty[i]["solution"].stringValue) {
                        // Create question
